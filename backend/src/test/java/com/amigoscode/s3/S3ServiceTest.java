...
class S3ServiceTest {
    ...
    @Test
    void canPutObejct() throws IOException {
        String bucket = "customer";
        String key = "foo_test";
        byte[] file = "Hello world".getBytes();

        underTest.putObejct(bucket, key, file);
        ...
        assertThat(
                requestBodyArgumentCaptorValue.contentStreamProvider()
                        .newStream()
                        .readAllBytes()
        ).isEqualTo(
                RequestBody.fromBytes(file).contentStreamProvider()
                        .newStream()
                        .readAllBytes()
        );
    }
    ...
    void canGetObject() throws IOException {
        ...
        when(s3Client.getObject(eq(getObjectAclRequest))).thenReturn(res);
